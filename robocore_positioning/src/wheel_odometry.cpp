...
#include "robocore_positioning/wheel_odometry.hpp"
#include "tf2/utils.h"
...
OdometryMotionModel::OdometryMotionModel(const std::string &name)
    ...
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>("robocore_control/odom", 10,
        std::bind(&OdometryMotionModel::odomCallback, this, _1));
    pose_array_pub_ = create_publisher<geometry_msgs::msg::PoseArray>("wheel_odometry/samples", 10);
}
...
int main(int argc, char* argv[]) {
  ...
  auto node = std::make_shared<OdometryMotionModel>("wheel_odometry");
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
