...
class NoisyController(Node):
  ...
  def __init__(self):
    super().__init__("filtered_controller")
    ...
    self.odom_pub_ = self.create_publisher(Odometry, "robocore_control/odom_noisy", 10)
...
def main():
  ...
  filtered_controller = NoisyController()
  rclpy.spin(filtered_controller)
  
  filtered_controller.destroy_node()
  rclpy.shutdown()
