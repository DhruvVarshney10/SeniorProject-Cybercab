#include "robocore_cpp_demos/transform_calculator.hpp"
...
SimpleTfKinematics::SimpleTfKinematics(const std::string& name)
  ...
  // Service Server
  get_transform_srv_ = create_service<robocore_interfaces::srv::GetTransform>("get_transform", std::bind(&SimpleTfKinematics::getTransformCallback, this, _1, _2));
...
bool SimpleTfKinematics::getTransformCallback(const std::shared_ptr<robocore_interfaces::srv::GetTransform::Request> req,
                                              const std::shared_ptr<robocore_interfaces::srv::GetTransform::Response> res) {
...
int main(int argc, char* argv[]) {
  ...
  auto node = std::make_shared<SimpleTfKinematics>("transform_calculator");
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}